...
LATEST_BUILD=$(curl \
	...
	cut -d . -f 1 |
	sort -r | head -n 1)
echo "# Highest available build number for IntelliJ IDEA Ultimate / Community edition and PyCharm Professional / PyCharm Community edition" >jetbrains.lastBuild.txt
echo "${LATEST_BUILD}" >>jetbrains.lastBuild.txt
